function counterFacts(wₙ,v̄ₙ,Lₙ,Rₙ,πₙᵢ,λₙᵢ; Âᵢ=ones(size(wₙ)),B̂ₙᵢ=ones(size(πₙᵢ)),κ̂ₙᵢ=ones(size(πₙᵢ)),d̂ₙᵢ=ones(size(πₙᵢ)))
    "
    ...
    # defining main loop
    err_ŵₙ=err_λ̂ₙᵢ=1e4; tol = 1e-6; x = 1; 
    ...
    while (err_ŵₙ>=tol) | (err_λ̂ₙᵢ>=tol)
        "
        All of this updating equations can be found in SW appendix A.2. or  MRRH2 appendix B 
        "
        # calculate v̄̂ₙ
        v̄̂ₙ = (1 ./ v̄ₙ) .* sum(num_v̄̂ₙ(λₙᵢ,ŵₙ0,B̂ₙᵢ,κ̂ₙᵢ)./sum(num_v̄̂ₙ(λₙᵢ,ŵₙ0,B̂ₙᵢ,κ̂ₙᵢ),dims=2),dims=2) .* (ŵₙ0 .* wₙ) 
        ...
        err_ŵₙ = round(maximum(abs.(ŵₙ1-ŵₙ0)),digits=6)
        err_λ̂ₙᵢ = round(maximum(abs.(λ̂ₙᵢ1-λ̂ₙᵢ0)),digits=6)
        ...
        println([x, round(err_ŵₙ*1e6), round(err_λ̂ₙᵢ*1e6)])
        x+=1
